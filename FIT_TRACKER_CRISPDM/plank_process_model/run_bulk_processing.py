...
def main():
    """
    Encuentra y procesa todos los videos .mov en el directorio de datasets de Plancha,
    y guarda los resultados en una estructura de carpetas paralela.
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_path = os.path.dirname(script_dir)
    
    # Directorio raíz de los videos de entrada (específico para Plancha)
    root_input_dir = os.path.join(base_path, 'Dataset_Ejercicios', 'plank')
    
    # Directorio raíz donde se guardarán los resultados
    root_output_dir = os.path.join(base_path, 'resultados_analisis', 'plank')

    print("-" * 60)
    print(f"Iniciando procesamiento masivo de videos de PLANCHAS...")
    ...
    for dirpath, _, filenames in os.walk(root_input_dir):
        for filename in filenames:
            ...
            if filename.lower().endswith('.mov'):
                ...
                try:
                    print(f"\nProcesando video: {filename} en {relative_path}")
                    process_single_video(video_input_path, video_output_path, csv_output_path)
                except Exception as e:
                    print(f"!!! ERROR al procesar {filename}: {e}")
                    continue # Continúa con el siguiente video

    print("-" * 60)
    print("Procesamiento masivo de planchas completado.")
    print("-" * 60)
